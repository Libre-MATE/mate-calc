...
static int variable_is_defined(ParserState *state, const char *name) {
  /* FIXME: Make more generic */
  if (strcmp(name, "e") == 0 || strcmp(name, "i") == 0 ||
      strcmp(name, "π") == 0 || strcmp(name, "pi") == 0 ||
      strcmp(name, "c₀") == 0 || strcmp(name, "μ₀") == 0 ||
      strcmp(name, "ε₀") == 0 || strcmp(name, "G") == 0 ||
      strcmp(name, "h") == 0 || strcmp(name, "ｅ") == 0 ||
      strcmp(name, "mₑ") == 0 || strcmp(name, "mₚ") == 0 ||
      strcmp(name, "Nₐ") == 0)
    return 1;
  if (state->options->variable_is_defined)
    return state->options->variable_is_defined(name,
                                               state->options->callback_data);
  return 0;
}

static int get_variable(ParserState *state, const char *name, MPNumber *z) {
  int result = 1;
  ...
  if (strcmp(name, "e") == 0)
    ...
    mp_set_from_string("0.0000000000667408", 10,
                       z); /* Newtonian constant of gravitation */
  else if (strcmp(name, "h") == 0)
    mp_set_from_string("0.000000000000000000000000000000000662607004", 10,
                       z); /* Planck constant */
  else if (strcmp(name, "ｅ") == 0)
    mp_set_from_string("0.00000000000000000016021766208", 10,
                       z); /* elementary charge */
  else if (strcmp(name, "mₑ") == 0)
    mp_set_from_string("0.000000000000000000000000000000910938356", 10,
                       z); /* electron mass */
  else if (strcmp(name, "mₚ") == 0)
    mp_set_from_string("0.000000000000000000000000001672621898", 10,
                       z); /* proton mass */
  else if (strcmp(name, "Nₐ") == 0)
    mp_set_from_string("602214086000000000000000", 10,
                       z); /* Avogadro constant */
  else if (state->options->get_variable)
    result =
        state->options->get_variable(name, z, state->options->callback_data);
  else
    result = 0;
...
static void set_variable(ParserState *state, const char *name,
                         const MPNumber *x) {
  // Reserved words, e, π, mod, and, or, xor, not, abs, log, ln, sqrt, int,
  // frac, sin, cos, ...
  if (strcmp(name, "e") == 0 || strcmp(name, "i") == 0 ||
      strcmp(name, "π") == 0 || strcmp(name, "pi") == 0 ||
      strcmp(name, "c₀") == 0 || strcmp(name, "μ₀") == 0 ||
...
// Parser then distinguishes between "sin"="s*i*n" or "sin5" = "sin 5" =
// "sin(5)" i.e. numbers+letters = variable or function depending on following
// arg letters+numbers = numbers+letters+numbers = function
...
int sub_atoi(const char *data) {
  ...
  do {
    for (i = 0;
         digits[i] != NULL && strncmp(data, digits[i], strlen(digits[i])) != 0;
         i++)
      ;
...
static int function_is_defined(ParserState *state, const char *name) {
  char *c, *lower_name;
  ...
  if (strcmp(lower_name, "log") == 0 ||
      ...
      strcmp(lower_name, "ln") == 0 || strcmp(lower_name, "sqrt") == 0 ||
      strcmp(lower_name, "abs") == 0 || strcmp(lower_name, "sgn") == 0 ||
      strcmp(lower_name, "arg") == 0 || strcmp(lower_name, "conj") == 0 ||
      strcmp(lower_name, "int") == 0 || strcmp(lower_name, "frac") == 0 ||
      strcmp(lower_name, "floor") == 0 || strcmp(lower_name, "ceil") == 0 ||
      strcmp(lower_name, "round") == 0 || strcmp(lower_name, "re") == 0 ||
      strcmp(lower_name, "im") == 0 || strcmp(lower_name, "sin") == 0 ||
      strcmp(lower_name, "cos") == 0 || strcmp(lower_name, "tan") == 0 ||
      strcmp(lower_name, "asin") == 0 || strcmp(lower_name, "acos") == 0 ||
      strcmp(lower_name, "atan") == 0 || strcmp(lower_name, "sin⁻¹") == 0 ||
      strcmp(lower_name, "cos⁻¹") == 0 || strcmp(lower_name, "tan⁻¹") == 0 ||
      strcmp(lower_name, "sinh") == 0 || strcmp(lower_name, "cosh") == 0 ||
      strcmp(lower_name, "tanh") == 0 || strcmp(lower_name, "sinh⁻¹") == 0 ||
      strcmp(lower_name, "cosh⁻¹") == 0 || strcmp(lower_name, "tanh⁻¹") == 0 ||
      strcmp(lower_name, "erf") == 0 || strcmp(lower_name, "zeta") == 0 ||
      strcmp(lower_name, "asinh") == 0 || strcmp(lower_name, "acosh") == 0 ||
      strcmp(lower_name, "atanh") == 0 || strcmp(lower_name, "ones") == 0 ||
...
static int get_function(ParserState *state, const char *name, const MPNumber *x,
                        MPNumber *z) {
  char *c, *lower_name;
  int result = 1;
  ...
  if (strcmp(lower_name, "log") == 0)
    ...
    mp_tanh(x, z);
           ...
           strcmp(lower_name, "asinh") == 0)
    mp_asinh(x, z);
  else if (strcmp(lower_name, "cosh⁻¹") == 0 ||
           strcmp(lower_name, "acosh") == 0)
    mp_acosh(x, z);
  else if (strcmp(lower_name, "tanh⁻¹") == 0 ||
           strcmp(lower_name, "atanh") == 0)
    mp_atanh(x, z);
    ...
    result =
        state->options->get_function(name, x, z, state->options->callback_data);
  else
    result = 0;
...
static int convert(ParserState *state, const MPNumber *x, const char *x_units,
                   const char *z_units, MPNumber *z) {
  if (state->options->convert)
    return state->options->convert(x, x_units, z_units, z,
                                   state->options->callback_data);
  else
    return 0;
}

MPErrorCode mp_equation_parse(const char *expression,
                              MPEquationOptions *options, MPNumber *result,
                              char **error_token) {
  int ret;
  int err;
  ParserState *state;
