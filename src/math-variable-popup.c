...
G_DEFINE_TYPE_WITH_PRIVATE(MathVariablePopup, math_variable_popup,
                           GTK_TYPE_WINDOW);

MathVariablePopup *math_variable_popup_new(MathEquation *equation) {
  return g_object_new(math_variable_popup_get_type(), "equation", equation,
                      NULL);
}

static void variable_focus_out_event_cb(GtkWidget *widget, GdkEventFocus *event,
                                        MathVariablePopup *popup) {
  gtk_widget_destroy(widget);
}

static void insert_variable_cb(GtkWidget *widget, MathVariablePopup *popup) {
  const gchar *name;
...
static gboolean variable_name_key_press_cb(GtkWidget *widget,
                                           GdkEventKey *event,
                                           MathVariablePopup *popup) {
...
static void variable_name_changed_cb(GtkWidget *widget,
                                     MathVariablePopup *popup) {
  const gchar *text =
      gtk_entry_get_text(GTK_ENTRY(popup->priv->variable_name_entry));
  gtk_widget_set_sensitive(popup->priv->add_variable_button, text[0] != '\0');
}

static void add_variable_cb(GtkWidget *widget, MathVariablePopup *popup) {
  const gchar *name;
  MPNumber z = mp_new();
  ...
  if (math_equation_get_number(popup->priv->equation, &z))
    math_variables_set(math_equation_get_variables(popup->priv->equation), name,
                       &z);
  else if (math_equation_is_result(popup->priv->equation))
    math_variables_set(math_equation_get_variables(popup->priv->equation), name,
                       math_equation_get_answer(popup->priv->equation));
  else
...
static void save_variable_cb(GtkWidget *widget, MathVariablePopup *popup) {
  const gchar *name;
  MPNumber z = mp_new();
...
static void delete_variable_cb(GtkWidget *widget, MathVariablePopup *popup) {
  const gchar *name;

  name = g_object_get_data(G_OBJECT(widget), "variable_name");
  math_variables_delete(math_equation_get_variables(popup->priv->equation),
                        name);

  gtk_widget_destroy(gtk_widget_get_toplevel(widget));
}

static GtkWidget *make_variable_entry(MathVariablePopup *popup,
                                      const gchar *name, const MPNumber *value,
                                      gboolean writable) {
  ...
  if (value) {
    gchar *value_text = mp_serializer_to_string(
        math_equation_get_serializer(popup->priv->equation), value);
  ...
  g_object_set_data(G_OBJECT(button), "variable_name",
                    g_strdup(name));  // FIXME: These will all leak memory
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(insert_variable_cb),
                   popup);
  ...
  if (writable) {
    ...
    g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(save_variable_cb),
                     popup);
    ...
    g_signal_connect(G_OBJECT(button), "clicked",
                     G_CALLBACK(delete_variable_cb), popup);
    gtk_widget_show(image);
    gtk_widget_show(button);
  }
...
static void math_variable_popup_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
  ...
  switch (prop_id) {
    case PROP_EQUATION:
      ...
      names = math_variables_get_names(
          math_equation_get_variables(self->priv->equation));
      for (i = 0; names[i]; i++) {
        MPNumber *aux;

        aux = math_variables_get(
            math_equation_get_variables(self->priv->equation), names[i]);
      ...
      g_signal_connect(G_OBJECT(self->priv->variable_name_entry),
                       "key-press-event",
                       G_CALLBACK(variable_name_key_press_cb), self);
      g_signal_connect(G_OBJECT(self->priv->variable_name_entry), "changed",
                       G_CALLBACK(variable_name_changed_cb), self);
      g_signal_connect(G_OBJECT(self->priv->variable_name_entry), "activate",
                       G_CALLBACK(add_variable_cb), self);
      gtk_box_pack_start(GTK_BOX(entry), self->priv->variable_name_entry, TRUE,
                         TRUE, 0);
      ...
      g_signal_connect(G_OBJECT(self->priv->add_variable_button), "clicked",
                       G_CALLBACK(add_variable_cb), self);
      ...
      gtk_box_pack_start(GTK_BOX(entry), self->priv->add_variable_button, FALSE,
                         TRUE, 0);
      gtk_widget_show(image);
      ...
      entry = make_variable_entry(
          self, "ans", math_equation_get_answer(self->priv->equation), FALSE);
      gtk_widget_show(entry);
      gtk_box_pack_end(GTK_BOX(self->priv->vbox), entry, FALSE, TRUE, 0);
      break;
    default:
...
static void math_variable_popup_get_property(GObject *object, guint prop_id,
...
static void math_variable_popup_init(MathVariablePopup *popup) {
  ...
  g_signal_connect(G_OBJECT(popup), "focus-out-event",
                   G_CALLBACK(variable_focus_out_event_cb), popup);
