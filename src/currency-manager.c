...
CurrencyManager *currency_manager_get_default(void) {
  ...
  if (default_currency_manager) return default_currency_manager;

  default_currency_manager = g_object_new(currency_manager_get_type(), NULL);

  for (i = 0; currency_info[i].short_name; i++) {
    Currency *c =
        currency_new(currency_info[i].short_name, _(currency_info[i].long_name),
                     currency_info[i].symbol);
    default_currency_manager->priv->currencies =
        g_list_append(default_currency_manager->priv->currencies, c);
  }
...
Currency *currency_manager_get_currency(CurrencyManager *manager,
  ...
  for (link = manager->priv->currencies; link; link = link->next) {
    ...
    if (!strcmp(name, currency_get_name(c))) {
      if (mp_is_negative(value) || mp_is_zero(value)) {
        return NULL;
      } else
        return c;
    }
...
static char *get_imf_rate_filepath(void) {
  return g_build_filename(g_get_user_cache_dir(), "mate-calc", "rms_five.xls",
                          NULL);
}

static char *get_ecb_rate_filepath(void) {
  return g_build_filename(g_get_user_cache_dir(), "mate-calc",
                          "eurofxref-daily.xml", NULL);
}

static Currency *add_currency(CurrencyManager *manager,
  ...
  for (iter = manager->priv->currencies; iter; iter = iter->next) {
    c = iter->data;
    if (strcmp(short_name, currency_get_name(c)) == 0) return c;
  }
 ...
 */
static gboolean file_needs_update(gchar *filename, double max_age) {
  struct stat buf;

  if (!g_file_test(filename, G_FILE_TEST_IS_REGULAR)) return TRUE;

  if (g_stat(filename, &buf) == -1) return TRUE;

  if (difftime(time(NULL), buf.st_mtime) > max_age) return TRUE;

  return FALSE;
}

static void download_imf_cb(GObject *object, GAsyncResult *result,
                            gpointer user_data) {
...
static void download_ecb_cb(GObject *object, GAsyncResult *result,
                            gpointer user_data) {
  CurrencyManager *manager = user_data;
  GError *error = NULL;
...
static void download_file(CurrencyManager *manager, gchar *uri, gchar *filename,
  ...
  g_file_copy_async(source, dest, G_FILE_COPY_OVERWRITE, G_PRIORITY_DEFAULT,
                    NULL, NULL, NULL, callback, manager);
  g_object_unref(source);
  g_object_unref(dest);
}

static void load_imf_rates(CurrencyManager *manager) {
  gchar *filename;
  gchar *data, **lines;
  gsize length;
  GError *error = NULL;
  ...
  } name_map[] = {{"Algerian dinar", "DZD"},      {"Australian dollar", "AUD"},
    ...
    if (strcmp(tokens[0], "Currency") != 0) {
      ...
      if (tokens[value_index]) {
        for (name_index = 0; name_map[name_index].name; name_index++) {
          if (strcmp(name_map[name_index].name, tokens[0]) == 0) break;
        }
        if (name_map[name_index].name) {
          Currency *c = currency_manager_get_currency(
              manager, name_map[name_index].symbol);
          MPNumber value = mp_new();

          if (!c) {
            g_debug("Using IMF rate of %s for %s", tokens[value_index],
                    name_map[name_index].symbol);
...
static void set_ecb_rate(CurrencyManager *manager, xmlNodePtr node,
                         Currency *eur_rate) {
...
static void set_ecb_fixed_rate(CurrencyManager *manager, const gchar *name,
                               const gchar *value, Currency *eur_rate) {
  Currency *c;
  MPNumber r = mp_new();
  MPNumber v = mp_new();
...
static void load_ecb_rates(CurrencyManager *manager) {
  Currency *eur_rate;
  char *filename;
  xmlDocPtr document;
  ...
  if (!document) g_warning("Couldn't parse ECB rate file %s", filename);
  g_free(filename);
  if (!document) return;
...
static gboolean load_rates(CurrencyManager *manager) {
  ...
  /* Already loaded */
  if (loaded_rates) return TRUE;

  /* In process */
  if (downloading_imf_rates || downloading_ecb_rates) return FALSE;

  /* Use the IMF provided values and top up with currencies tracked by the ECB
   * and not the IMF */
  ...
  for (i = 0; currency_info[i].short_name; i++) {
    ...
    for (link = manager->priv->currencies; link; link = link->next) {
      Currency *c = link->data;
      if (strcmp(currency_get_name(c), currency_info[i].short_name) == 0) break;
    }
    if (!link)
      g_warning("Currency %s is not provided by IMF or ECB",
                currency_info[i].short_name);
  }
...
const MPNumber *currency_manager_get_value(CurrencyManager *manager,
  ...
  if (!downloading_ecb_rates && file_needs_update(path, 60 * 60 * 24 * 7)) {
    ...
    download_file(
        manager, "http://www.ecb.europa.eu/stats/eurofxref/eurofxref-daily.xml",
        path, download_ecb_cb);
  }
  g_free(path);

  if (!load_rates(manager)) return NULL;
...
static void currency_manager_class_init(CurrencyManagerClass *klass) {
  signals[UPDATED] =
      g_signal_new("updated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CurrencyManagerClass, updated), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void currency_manager_init(CurrencyManager *manager) {
