...
void set_error(ParserState* state, gint errorno, const gchar* token) {
  state->error = errorno;
  if (token) state->error_token = strdup(token);
}

/* Unused function pointer. This won't be called anytime. */
void* pf_none(ParseNode* self) { return NULL; }

/* Set variable. */
void* pf_set_var(ParseNode* self) {
  MPNumber* val;
...
void* pf_convert_number(ParseNode* self) {
  gchar* from;
  gchar* to;
  gint free_from = 0;
  gint free_to = 0;
  ...
  if (mp_set_from_string(self->left->left->token->string,
                         self->state->options->base, &tmp) != 0) {
    mp_free(ans);
    ans = NULL;
...
/* Conversion rate. */
void* pf_convert_1(ParseNode* self) {
  gchar* from;
  gchar* to;
  gint free_from = 0;
  gint free_to = 0;
...
void* pf_get_variable(ParseNode* self) {
  gint result = 0;
  ...
  if (utf8_next_char(self->token->string)[0] != '\0') {
    ...
    free(buffer);
    if (result) mp_set_from_mp(&value, ans);
  }
  if (!result) {
    free(ans);
    ans = NULL;
...
void* pf_get_variable_with_power(ParseNode* self) {
  gint result = 0;
  gint pow;
  ...
  if (utf8_next_char(self->token->string)[0] != '\0') {
    ...
    for (c = self->token->string; *c != '\0'; c = next) {
      ...
      if (*next == '\0') mp_xpowy_integer(&t, pow, &t);
      mp_multiply(&value, &t, &value);
    }
    free(buffer);
    if (result) mp_set_from_mp(&value, ans);
...
void* pf_apply_func(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  ...
  if (!(*(self->state->get_function))(self->state, self->token->string, val,
                                      ans)) {
    free(val);
    free(ans);
...
void* pf_apply_func_with_power(ParseNode* self) {
  MPNumber* val;
...
/* Apply function with -ve power. */
void* pf_apply_func_with_npower(ParseNode* self) {
  MPNumber* val;
  MPNumber* tmp = mp_new_ptr();
  MPNumber* ans = mp_new_ptr();
  gint pow;
  gchar* inv_name;
  inv_name = (gchar*)malloc(sizeof(gchar) * strlen(self->token->string) +
                            strlen("⁻¹") + 1);
...
void* pf_do_nth_root(ParseNode* self) {
  MPNumber* val;
  gint pow;
  MPNumber* ans = mp_new_ptr();
...
/* Find sqrt of child. */
void* pf_do_sqrt(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
void* pf_do_root_3(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
void* pf_do_root_4(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
void* pf_do_floor(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
/* Round off. */
void* pf_do_round(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
/* Fraction. */
void* pf_do_fraction(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
void* pf_do_x_pow_y(ParseNode* self) {
  MPNumber* val;
  MPNumber* pow;
  MPNumber* ans = mp_new_ptr();
...
void* pf_do_x_pow_y_int(ParseNode* self) {
  MPNumber* val;
  long pow;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->left->evaluate))(self->left);
...
/* Find factorial. */
void* pf_do_factorial(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
/* Apply unary minus. */
void* pf_unary_minus(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
/* Divide. */
void* pf_do_divide(ParseNode* self) {
  MPNumber* left;
  MPNumber* right;
  MPNumber* ans = mp_new_ptr();
  ...
  if (!left || !right) {
    if (left) mp_free(left);
    if (right) mp_free(right);
    mp_free(ans);
    return NULL;
  }
...
void* pf_do_mod(ParseNode* self) {
  ...
  if (self->left->evaluate == pf_do_x_pow_y) {
    MPNumber* base_value =
        (MPNumber*)(*(self->left->left->evaluate))(self->left->left);
    MPNumber* exponent =
        (MPNumber*)(*(self->left->right->evaluate))(self->left->right);
    if (!base_value || !exponent) {
      if (base_value) mp_free(base_value);
      if (exponent) mp_free(exponent);
      mp_free(ans);
      return NULL;
    }
...
void* pf_do_multiply(ParseNode* self) {
  MPNumber* left;
  MPNumber* right;
  MPNumber* ans = mp_new_ptr();
  left = (MPNumber*)(*(self->left->evaluate))(self->left);
...
void* pf_do_subtract(ParseNode* self) {
  MPNumber* left;
  MPNumber* right;
  MPNumber* ans = mp_new_ptr();
  left = (MPNumber*)(*(self->left->evaluate))(self->left);
...
void* pf_do_percent(ParseNode* self) {
  MPNumber* val;
  MPNumber* ans = mp_new_ptr();
  val = (MPNumber*)(*(self->right->evaluate))(self->right);
  if (!val) {
...
void* pf_constant(ParseNode* self) {
  MPNumber* ans = mp_new_ptr();
  if (mp_set_from_string(self->token->string, self->state->options->base,
                         ans) != 0) {
    /* This should never happen, as l_check_if_number() has already passed the
     * string once. */
