...
MpSerializer *mp_serializer_new(MpDisplayFormat format, int base,
                                int trailing_digits) {
...
static void mp_to_string_real(MpSerializer *serializer, const MPNumber *x,
                              int base, gboolean force_sign, int *n_digits,
                              GString *string) {
  ...
  if (!mp_get_error()) mp_set_from_mp(&temp, &number);
  ...
  do {
    ...
    if (serializer->priv->base == 10 && serializer->priv->show_tsep &&
        i == serializer->priv->tsep_count) {
    ...
    else { /* FIXME: Should show an overflow error message in fixed mode */
  ...
  for (i = serializer->priv->trailing_digits; i > 0 && !mp_is_zero(&temp);
       i--) {
    int d;
    ...
    if (d != 0) last_non_zero = string->len;
  ...
  if (base != serializer->priv->base) {
    ...
    while (base / multiplier != 0) multiplier *= 10;
    while (multiplier != 1) {
      int d;
      multiplier /= 10;
      d = b / multiplier;
...
static gchar *mp_to_string(MpSerializer *serializer, const MPNumber *x,
                           int *n_digits) {
  GString *string;
  ...
  mp_to_string_real(serializer, &x_real, serializer->priv->base, FALSE,
                    n_digits, string);
  if (mp_is_complex(x)) {
    GString *s;
    ...
    if (n_complex_digits > *n_digits) *n_digits = n_complex_digits;
    if (strcmp(s->str, "0") == 0 || strcmp(s->str, "+0") == 0 ||
        strcmp(s->str, "−0") == 0) {
...
static int mp_to_exponential_string_real(MpSerializer *serializer,
                                         const MPNumber *x, GString *string,
                                         gboolean eng_format, int *n_digits) {
  gchar *fixed;
  MPNumber t = mp_new();
  MPNumber z = mp_new();
  ...
  mp_abs(x, &z);
  if (mp_is_negative(x)) g_string_append(string, "−");
  ...
  if (!mp_is_zero(&mantissa)) {
    ...
    while ((!eng_format && mp_is_greater_equal(&mantissa, &base)) ||
           (eng_format &&
            (mp_is_greater_equal(&mantissa, &base3) || exponent % 3 != 0))) {
      exponent += 1;
    ...
    mp_set_from_integer(1, &t);
    while (mp_is_less_than(&mantissa, &t) ||
           (eng_format && exponent % 3 != 0)) {
      exponent -= 1;
...
static void append_exponent(GString *string, int exponent) {
...
static gchar *mp_to_exponential_string(MpSerializer *serializer,
                                       const MPNumber *x, gboolean eng_format,
                                       int *n_digits) {
  GString *string;
  MPNumber x_real = mp_new();
  gchar *result;
  ...
  mp_real_component(x, &x_real);
  exponent = mp_to_exponential_string_real(serializer, &x_real, string,
                                           eng_format, n_digits);
  ...
  if (mp_is_complex(x)) {
    ...
    s = g_string_sized_new(1024);
    exponent = mp_to_exponential_string_real(serializer, &x_im, s, eng_format,
                                             &n_complex_digits);
    if (n_complex_digits > *n_digits) *n_digits = n_complex_digits;
    if (strcmp(s->str, "0") == 0 || strcmp(s->str, "+0") == 0 ||
        strcmp(s->str, "−0") == 0) {
...
gchar *mp_serializer_to_string(MpSerializer *serializer, const MPNumber *x) {
  gchar *s0;
  int n_digits = 0;
...
gboolean mp_serializer_from_string(MpSerializer *serializer, const gchar *str,
                                   MPNumber *z) {
  return mp_set_from_string(str, serializer->priv->base, z);
}

void mp_serializer_set_base(MpSerializer *serializer, gint base) {
...
void mp_serializer_set_thousands_separator(MpSerializer *serializer,
                                           gunichar separator) {
...
void mp_serializer_set_show_thousands_separators(MpSerializer *serializer,
                                                 gboolean visible) {
...
void mp_serializer_set_show_trailing_zeroes(MpSerializer *serializer,
                                            gboolean visible) {
...
void mp_serializer_set_leading_digits(MpSerializer *serializer,
                                      int leading_digits) {
...
void mp_serializer_set_trailing_digits(MpSerializer *serializer,
                                       int trailing_digits) {
...
void mp_serializer_set_number_format(MpSerializer *serializer,
                                     MpDisplayFormat format) {
  serializer->priv->format = format;
}

static void mp_serializer_set_property(GObject *object, guint prop_id,
                                       const GValue *value, GParamSpec *pspec) {
  MpSerializer *self = MP_SERIALIZER(object);

  switch (prop_id) {
    case PROP_SHOW_THOUSANDS_SEPARATORS:
      mp_serializer_set_show_thousands_separators(self,
...
static void mp_serializer_get_property(GObject *object, guint prop_id,
...
static void mp_serializer_class_init(MpSerializerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("number-format", "number-format", "Display format",
                        ...
                        MP_DISPLAY_FORMAT_AUTOMATIC, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_BASE,
      g_param_spec_int("base", "base", "Default number base", 2, 16, 10,
                       G_PARAM_READWRITE));
}

static void mp_serializer_init(MpSerializer *serializer) {
