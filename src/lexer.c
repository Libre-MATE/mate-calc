...
static gboolean l_check_if_function(LexerState* state) {
...
static gboolean l_check_if_number(LexerState* state) {
  MPNumber tmp = mp_new();
  int count = 0;
...
static LexerToken* l_insert_token(LexerState* state,
                                  const LexerTokenType type) {
  state->tokens = (LexerToken*)realloc(
      state->tokens, (state->token_count + 1) * sizeof(LexerToken));
  assert(state->tokens != NULL);
  state->tokens[state->token_count].string =
      pl_get_marked_substring(state->prelexer);
...
/* Generates next token from pre-lexer stream and call l_insert_token() to
 * insert it at the end. */
static LexerToken* l_insert_next_token(LexerState* lstate) {
  ...
  while ((type = pl_get_next_token(state)) == PL_SKIP)
    ...
    pl_set_marker(state);
  if (type == T_AND || type == T_OR || type == T_XOR || type == T_NOT ||
      type == T_ADD || type == T_SUBTRACT || type == T_MULTIPLY ||
      type == T_DIV || type == T_L_FLOOR || type == T_R_FLOOR ||
      type == T_L_CEILING || type == T_R_CEILING || type == T_ROOT ||
      type == T_ROOT_3 || type == T_ROOT_4 || type == T_ASSIGN ||
      type == T_L_R_BRACKET || type == T_R_R_BRACKET || type == T_L_S_BRACKET ||
      type == T_R_S_BRACKET || type == T_L_C_BRACKET || type == T_R_C_BRACKET ||
      type == T_ABS || type == T_POWER || type == T_FACTORIAL ||
      type == T_PERCENTAGE) {
  ...
  if (type == PL_SUPER_MINUS) {
    if ((type = pl_get_next_token(state)) != PL_SUPER_DIGIT) {
      ...
      set_error(lstate->parent, PARSER_ERR_MP,
                tmp = pl_get_marked_substring(state));
      free(tmp);
  ...
  set_error(lstate->parent, PARSER_ERR_INVALID,
            tmp = pl_get_marked_substring(state));
  free(tmp);
  return l_insert_token(lstate, T_UNKNOWN);
}

/* Call l_insert_next_token() as many times as needed to completely tokenize the
 * string. */
...
/* Create a lexer state from given input string. This will take care of
 * pre-lexer state. */
LexerState* l_create_lexer(const gchar* input, struct parser_state* parent) {
  LexerState* ret;
...
/* Get next token interface. Will be called by parser to get pointer to next
 * token in token stream. */
...
void l_roll_back(LexerState* state) {
  if (state->next_token > 0) state->next_token--;
}
