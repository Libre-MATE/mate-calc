...
static void version(const gchar *progname) {
...
static void usage(const gchar *progname, gboolean show_application,
                  gboolean show_gtk) {
  fprintf(stderr,
  ...
  fprintf(stderr,
          /* Description on mate-calc command-line help options displayed on
             command-line */
          _("Help Options:\n"
  ...
  if (show_gtk) {
    fprintf(stderr,
            /* Description on mate-calc command-line GTK+ options displayed on
               command-line */
            _("GTK+ Options:\n"
  ...
  if (show_application) {
    fprintf(stderr,
            /* Description on mate-calc application options displayed on
               command-line */
...
static void get_options(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (strcmp(arg, "-v") == 0 || strcmp(arg, "--version") == 0) {
      ...
      if (i >= argc) {
        fprintf(stderr,
                /* Error printed to stderr when user uses --solve argument
                   without an equation */
      ...
      fprintf(stderr,
              /* Error printed to stderr when user provides an unknown
...
static void quit_cb(MathWindow *win) { gtk_main_quit(); }
...
int main(int argc, char **argv) {
  ...
  math_buttons_set_mode(
      buttons, button_mode);  // FIXME: We load the basic buttons even if we
                              // immediately switch to the next type
