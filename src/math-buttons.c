...
enum { PROP_0, PROP_EQUATION, PROP_MODE, PROP_PROGRAMMING_BASE };
...
#define UI_FINANCIAL_RESOURCE_PATH \
  "/org/mate/calculator/ui/buttons-financial.ui"
#define UI_PROGRAMMING_RESOURCE_PATH \
  "/org/mate/calculator/ui/buttons-programming.ui"
...
static void set_data(GtkBuilder *ui, const gchar *object_name,
                     const gchar *name, const char *value) {
  GObject *object;
  object = gtk_builder_get_object(ui, object_name);
  if (object) g_object_set_data(object, name, GINT_TO_POINTER(value));
}

static void set_int_data(GtkBuilder *ui, const gchar *object_name,
...
static void load_finc_dialogs(MathButtons *buttons) {
  int i, j;

  set_int_data(buttons->priv->financial_ui, "ctrm_dialog", "finc_dialog",
               FINC_CTRM_DIALOG);
  set_int_data(buttons->priv->financial_ui, "ddb_dialog", "finc_dialog",
               FINC_DDB_DIALOG);
  set_int_data(buttons->priv->financial_ui, "fv_dialog", "finc_dialog",
               FINC_FV_DIALOG);
  set_int_data(buttons->priv->financial_ui, "gpm_dialog", "finc_dialog",
               FINC_GPM_DIALOG);
  set_int_data(buttons->priv->financial_ui, "pmt_dialog", "finc_dialog",
               FINC_PMT_DIALOG);
  set_int_data(buttons->priv->financial_ui, "pv_dialog", "finc_dialog",
               FINC_PV_DIALOG);
  set_int_data(buttons->priv->financial_ui, "rate_dialog", "finc_dialog",
               FINC_RATE_DIALOG);
  set_int_data(buttons->priv->financial_ui, "sln_dialog", "finc_dialog",
               FINC_SLN_DIALOG);
  set_int_data(buttons->priv->financial_ui, "syd_dialog", "finc_dialog",
               FINC_SYD_DIALOG);
  set_int_data(buttons->priv->financial_ui, "term_dialog", "finc_dialog",
               FINC_TERM_DIALOG);

  for (i = 0; finc_dialog_fields[i][0] != NULL; i++) {
    for (j = 0; finc_dialog_fields[i][j]; j++) {
      GObject *o;
      o = gtk_builder_get_object(buttons->priv->financial_ui,
...
static void update_bit_panel(MathButtons *buttons) {
  MPNumber x;
  gboolean enabled;
  guint64 bits;
  int i;
  ...
  if (!buttons->priv->bit_panel) return;
  ...
  label = g_string_new("");
  if (base != 8) {
    if (label->len != 0) g_string_append(label, " = ");
...
static void display_changed_cb(MathEquation *equation, GParamSpec *spec,
                               MathButtons *buttons) {
  update_bit_panel(buttons);
}

static void base_combobox_changed_cb(GtkWidget *combo, MathButtons *buttons) {
  gint value;
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void base_changed_cb(MathEquation *equation, GParamSpec *spec,
                            MathButtons *buttons) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  gboolean valid;

  if (buttons->priv->mode != PROGRAMMING) return;

  model = gtk_combo_box_get_model(GTK_COMBO_BOX(buttons->priv->base_combo));
  valid = gtk_tree_model_get_iter_first(model, &iter);
  buttons->priv->programming_base =
      math_equation_get_base(buttons->priv->equation);

  while (valid) {
    gint v;

    gtk_tree_model_get(model, &iter, 1, &v, -1);
    if (v == buttons->priv->programming_base) break;
    valid = gtk_tree_model_iter_next(model, &iter);
  }
  if (!valid) valid = gtk_tree_model_get_iter_first(model, &iter);

  gtk_combo_box_set_active_iter(GTK_COMBO_BOX(buttons->priv->base_combo),
                                &iter);
}

static GtkWidget *load_mode(MathButtons *buttons, ButtonMode mode) {
  GtkBuilder *builder, **builder_ptr;
  gint i;
  gchar *name;
  const gchar *path;
  static gchar *objects[] = {"button_panel",    "character_code_dialog",
  ...
  *panel = GET_WIDGET(builder, "button_panel");
  ...
  for (i = 0; i < 16; i++) {
    ...
    if (button) {
      ...
      len = g_unichar_to_utf8(
          math_equation_get_digit_text(buttons->priv->equation, i), buffer);
      buffer[len] = '\0';
  ...
  if (widget) {
    MpSerializer *serializer =
        math_equation_get_serializer(buttons->priv->equation);
    gchar buffer[7];
    gint len;
  ...
  if (widget) {
    buttons->priv->superscript_toggles =
        g_list_append(buttons->priv->superscript_toggles, widget);
  ...
  if (widget) {
    buttons->priv->subscript_toggles =
        g_list_append(buttons->priv->subscript_toggles, widget);
  ...
  if (mode == PROGRAMMING) {
    ...
    buttons->priv->character_code_dialog =
        GET_WIDGET(builder, "character_code_dialog");
    buttons->priv->character_code_entry =
    ...
    gtk_combo_box_set_model(GTK_COMBO_BOX(buttons->priv->base_combo),
                            GTK_TREE_MODEL(model));
    gtk_list_store_append(GTK_LIST_STORE(model), &iter);
    gtk_list_store_set(
        GTK_LIST_STORE(model), &iter, 0,
    ...
    gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(buttons->priv->base_combo),
                               renderer, TRUE);
    gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(buttons->priv->base_combo),
                                  renderer, "text", 0);

    g_signal_connect(buttons->priv->base_combo, "changed",
                     G_CALLBACK(base_combobox_changed_cb), buttons);
    g_signal_connect(buttons->priv->equation, "notify::base",
                     G_CALLBACK(base_changed_cb), buttons);
  ...
  if (mode == FINANCIAL) {
    ...
    set_data(builder, "calc_finc_compounding_term_button", "finc_dialog",
             "ctrm_dialog");
    set_data(builder, "calc_finc_double_declining_depreciation_button",
             "finc_dialog", "ddb_dialog");
    set_data(builder, "calc_finc_future_value_button", "finc_dialog",
             "fv_dialog");
    set_data(builder, "calc_finc_gross_profit_margin_button", "finc_dialog",
             "gpm_dialog");
    set_data(builder, "calc_finc_periodic_payment_button", "finc_dialog",
             "pmt_dialog");
    set_data(builder, "calc_finc_present_value_button", "finc_dialog",
             "pv_dialog");
    set_data(builder, "calc_finc_periodic_interest_rate_button", "finc_dialog",
             "rate_dialog");
    set_data(builder, "calc_finc_straight_line_depreciation_button",
             "finc_dialog", "sln_dialog");
    set_data(builder, "calc_finc_sum_of_the_years_digits_depreciation_button",
             "finc_dialog", "syd_dialog");
...
static void converter_changed_cb(MathConverter *converter,
                                 MathButtons *buttons) {
  Unit *from_unit, *to_unit;

  math_converter_get_conversion(converter, &from_unit, &to_unit);
  if (buttons->priv->mode == FINANCIAL) {
    math_equation_set_source_currency(buttons->priv->equation,
                                      unit_get_name(from_unit));
    math_equation_set_target_currency(buttons->priv->equation,
                                      unit_get_name(to_unit));
  } else {
    math_equation_set_source_units(buttons->priv->equation,
                                   unit_get_name(from_unit));
    math_equation_set_target_units(buttons->priv->equation,
                                   unit_get_name(to_unit));
  }
...
static void load_buttons(MathButtons *buttons) {
  GtkWidget *panel;
  ...
  if (!buttons->priv->converter) {
    ...
    g_signal_connect(buttons->priv->converter, "changed",
                     G_CALLBACK(converter_changed_cb), buttons);
    gtk_box_pack_start(GTK_BOX(buttons), GTK_WIDGET(buttons->priv->converter),
                       FALSE, TRUE, 0);
  }
  ...
  /* Hide old buttons */
  if (buttons->priv->active_panel) gtk_widget_hide(buttons->priv->active_panel);
...
void math_buttons_set_mode(MathButtons *buttons, ButtonMode mode) {
  ...
  if (mode == PROGRAMMING)
    math_equation_set_base(buttons->priv->equation,
                           buttons->priv->programming_base);
  else
  ...
  gtk_widget_set_visible(GTK_WIDGET(buttons->priv->converter),
                         mode == ADVANCED || mode == FINANCIAL);
  if (mode == ADVANCED) {
    math_converter_set_category(buttons->priv->converter, NULL);
    math_converter_set_conversion(
        buttons->priv->converter,
...
void math_buttons_set_programming_base(MathButtons *buttons, gint base) {
  ...
  if (base == buttons->priv->programming_base) return;

  buttons->priv->programming_base = base;

  g_settings_set_int(g_settings_var, "base",
                     math_buttons_get_programming_base(buttons));
...
G_MODULE_EXPORT
void button_cb(GtkWidget *widget, MathButtons *buttons) {
  math_equation_insert(buttons->priv->equation,
...
static void shift_cb(GtkWidget *widget, MathButtons *buttons) {
  math_equation_shift(
      buttons->priv->equation,
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "shiftcount")));
}

static void popup_button_menu(GtkWidget *widget, GtkMenu *menu) {
  gtk_menu_popup_at_widget(menu, widget, GDK_GRAVITY_SOUTH_WEST,
...
void memory_cb(GtkWidget *widget, MathButtons *buttons) {
  ...
  gdk_window_get_root_coords(gtk_widget_get_window(widget), allocation.x,
                             allocation.y, &x, &y);
...
static void insert_function_cb(GtkWidget *widget, MathButtons *buttons) {
  math_equation_insert(buttons->priv->equation,
                       g_object_get_data(G_OBJECT(widget), "function"));
}
...
void function_cb(GtkWidget *widget, MathButtons *buttons) {
  if (!buttons->priv->function_menu) {
    ...
    struct {
      gchar *name, *function;
    } functions[] = {{/* Tooltip for the integer component button */
    ...
    for (i = 0; functions[i].name != NULL; i++) {
      ...
      g_object_set_data(G_OBJECT(item), "function",
                        g_strdup(functions[i].function));
      gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
      g_signal_connect(item, "activate", G_CALLBACK(insert_function_cb),
                       buttons);
      gtk_widget_show(item);
    }
  }
...
static void insert_const_cb(GtkWidget *widget, MathButtons *buttons) {
  math_equation_insert(buttons->priv->equation,
                       g_object_get_data(G_OBJECT(widget), "const"));
}
...
void const_cb(GtkWidget *widget, MathButtons *buttons) {
  if (!buttons->priv->const_menu) {
    ...
    for (i = 0; constants[i].name != NULL; i++) {
      ...
      g_object_set_data(G_OBJECT(item), "const",
                        g_strdup(constants[i].constant));
      gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
...
void finc_activate_cb(GtkWidget *widget, MathButtons *buttons) {
  ...
  if (finc_dialog_fields[dialog][field + 1] == NULL) {
    ...
    if (gtk_widget_is_toplevel(dialog_widget)) {
      gtk_dialog_response(GTK_DIALOG(dialog_widget), GTK_RESPONSE_OK);
      return;
    }
  } else {
    GtkWidget *next_widget;
    next_widget = GET_WIDGET(buttons->priv->financial_ui,
                             finc_dialog_fields[dialog][field + 1]);
    gtk_widget_grab_focus(next_widget);
  }
}

void finc_response_cb(GtkWidget *widget, gint response_id,
                      MathButtons *buttons);
G_MODULE_EXPORT
void finc_response_cb(GtkWidget *widget, gint response_id,
                      MathButtons *buttons) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    entry =
        GET_WIDGET(buttons->priv->financial_ui, finc_dialog_fields[dialog][i]);
    mp_set_from_string(gtk_entry_get_text(GTK_ENTRY(entry)), 10, &arg[i]);
    gtk_entry_set_text(GTK_ENTRY(entry), "0");
  }
  gtk_widget_grab_focus(
      GET_WIDGET(buttons->priv->financial_ui, finc_dialog_fields[dialog][0]));

  do_finc_expression(buttons->priv->equation, dialog, &arg[0], &arg[1], &arg[2],
                     &arg[3]);
...
void character_code_dialog_response_cb(GtkWidget *dialog, gint response_id,
                                       MathButtons *buttons);
G_MODULE_EXPORT
void character_code_dialog_response_cb(GtkWidget *dialog, gint response_id,
                                       MathButtons *buttons) {
...
G_MODULE_EXPORT
void character_code_dialog_activate_cb(GtkWidget *entry, MathButtons *buttons) {
  character_code_dialog_response_cb(buttons->priv->character_code_dialog,
                                    GTK_RESPONSE_OK, buttons);
}

gboolean character_code_dialog_delete_cb(GtkWidget *dialog, GdkEvent *event,
                                         MathButtons *buttons);
G_MODULE_EXPORT
gboolean character_code_dialog_delete_cb(GtkWidget *dialog, GdkEvent *event,
                                         MathButtons *buttons) {
...
gboolean bit_toggle_cb(GtkWidget *event_box, GdkEventButton *event,
                       MathButtons *buttons);
G_MODULE_EXPORT
gboolean bit_toggle_cb(GtkWidget *event_box, GdkEventButton *event,
                       MathButtons *buttons) {
  math_equation_toggle_bit(
      buttons->priv->equation,
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(event_box), "bit_index")));
  return TRUE;
}

static void remove_trailing_spaces(MathButtons *buttons) {
  GtkTextMark *insert_mark;
  GtkTextIter start, end;
  insert_mark =
      gtk_text_buffer_get_insert(GTK_TEXT_BUFFER(buttons->priv->equation));
  gtk_text_buffer_get_iter_at_mark(GTK_TEXT_BUFFER(buttons->priv->equation),
                                   &end, insert_mark);
  start = end;
  while (gtk_text_iter_backward_char(&start)) {
    if (!g_unichar_isspace(gtk_text_iter_get_char(&start))) break;
    gtk_text_buffer_delete(GTK_TEXT_BUFFER(buttons->priv->equation), &start,
                           &end);
  }
}
...
void set_superscript_cb(GtkWidget *widget, MathButtons *buttons) {
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
    ...
    if (!gtk_text_buffer_get_has_selection(
            GTK_TEXT_BUFFER(buttons->priv->equation))) {
      remove_trailing_spaces(buttons);
    }
  } else if (math_equation_get_number_mode(buttons->priv->equation) ==
             SUPERSCRIPT)
...
static void number_mode_changed_cb(MathEquation *equation, GParamSpec *spec,
                                   MathButtons *buttons) {
  GList *i;
  NumberMode mode;
...
static void math_buttons_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_EQUATION:
      ...
      g_signal_connect(self->priv->equation, "notify::display",
                       G_CALLBACK(display_changed_cb), self);
      g_signal_connect(self->priv->equation, "notify::number-mode",
                       G_CALLBACK(number_mode_changed_cb), self);
      g_signal_connect(self->priv->equation, "notify::angle-units",
                       G_CALLBACK(display_changed_cb), self);
      g_signal_connect(self->priv->equation, "notify::number-format",
                       G_CALLBACK(display_changed_cb), self);
...
static void math_buttons_get_property(GObject *object, guint prop_id,
...
static void math_buttons_class_init(MathButtonsClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("equation", "equation", "Equation being controlled",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("mode", "mode", "Button mode", button_mode_type, BASIC,
                        G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_PROGRAMMING_BASE,
      g_param_spec_int("programming-base", "programming-base",
                       "Base to use in programming mode", 2, 16, 10,
                       G_PARAM_READWRITE));
}

static void math_buttons_init(MathButtons *buttons) {
