...
MathConverter *math_converter_new(MathEquation *equation) {
  ...
  g_signal_connect(converter->priv->equation, "notify::display",
                   G_CALLBACK(display_changed_cb), converter);
  update_from_model(converter);
  return converter;
}

static gboolean convert_equation(MathConverter *converter, const MPNumber *x,
                                 MPNumber *z) {
  ...
  if (!gtk_combo_box_get_active_iter(GTK_COMBO_BOX(converter->priv->from_combo),
                                     &from_iter) ||
      !gtk_combo_box_get_active_iter(GTK_COMBO_BOX(converter->priv->to_combo),
                                     &to_iter))
    return FALSE;

  gtk_tree_model_get(
      gtk_combo_box_get_model(GTK_COMBO_BOX(converter->priv->from_combo)),
      &from_iter, 1, &category, 2, &source_unit, -1);
  gtk_tree_model_get(
      gtk_combo_box_get_model(GTK_COMBO_BOX(converter->priv->to_combo)),
      &to_iter, 2, &target_unit, -1);

  result = unit_category_convert(category, x, source_unit, target_unit, z);

  if (category) g_object_unref(category);
  if (source_unit) g_object_unref(source_unit);
  if (target_unit) g_object_unref(target_unit);

  return result;
}

static void update_result_label(MathConverter *converter) {
  MPNumber x = mp_new();
  MPNumber z = mp_new();
  gboolean enabled;

  if (!converter->priv->result_label) return;
...
static void update_from_model(MathConverter *converter) {
  ...
  from_model =
      gtk_tree_store_new(3, G_TYPE_STRING, G_TYPE_OBJECT, G_TYPE_OBJECT);
  ...
  if (converter->priv->category == NULL) {
    ...
    for (iter = categories; iter; iter = iter->next) {
      ...
      gtk_tree_store_set(from_model, &parent, 0,
                         unit_category_get_display_name(category), 1, category,
                         -1);

      for (unit_iter = unit_category_get_units(category); unit_iter;
        ...
        gtk_tree_store_set(from_model, &iter_new_row, 0,
                           unit_get_display_name(unit), 1, category, 2, unit,
                           -1);
      }
    }
  } else {
    ...
    category = unit_manager_get_category(unit_manager_get_default(),
                                         converter->priv->category);
    for (unit_iter = unit_category_get_units(category); unit_iter;
         unit_iter = unit_iter->next) {
      Unit *unit = unit_iter->data;
      GtkTreeIter iter;
      ...
      gtk_tree_store_set(from_model, &iter, 0, unit_get_display_name(unit), 1,
                         category, 2, unit, -1);
    }
  }

  gtk_combo_box_set_model(GTK_COMBO_BOX(converter->priv->from_combo),
...
void math_converter_set_category(MathConverter *converter,
  ...
  if (category == NULL && converter->priv->category == NULL) return;
  if (category != NULL && converter->priv->category != NULL &&
      strcmp(category, converter->priv->category) == 0)
    return;
...
static gboolean iter_is_unit(GtkTreeModel *model, GtkTreeIter *iter,
                             const Unit *unit) {
  Unit *u;
...
static gboolean set_active_unit(GtkComboBox *combo, GtkTreeIter *iter,
                                Unit *unit) {
  GtkTreeModel *model;
  ...
  if (!gtk_tree_model_iter_children(model, &child_iter, iter)) return FALSE;

  do {
    if (set_active_unit(combo, &child_iter, unit)) return TRUE;
  ...
  if (!ua || !ub) {
    ...
    if (gtk_tree_model_get_iter_first(model, &iter)) {
      ...
      while (gtk_tree_model_iter_children(model, &child_iter, &iter))
        iter = child_iter;
      gtk_combo_box_set_active_iter(GTK_COMBO_BOX(converter->priv->from_combo),
                                    &iter);
    }
    return;
  }
...
void math_converter_get_conversion(MathConverter *converter, Unit **from_unit,
  ...
  gtk_combo_box_get_active_iter(GTK_COMBO_BOX(converter->priv->from_combo),
                                &from_iter);
  gtk_combo_box_get_active_iter(GTK_COMBO_BOX(converter->priv->to_combo),
                                &to_iter);
...
static void math_converter_class_init(MathConverterClass *klass) {
  signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(MathConverterClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void from_combobox_changed_cb(GtkWidget *combo,
                                     MathConverter *converter) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  UnitCategory *category;
  ...
  model = GTK_TREE_MODEL(
      gtk_list_store_new(3, G_TYPE_STRING, G_TYPE_OBJECT, G_TYPE_OBJECT));
  for (unit_iter = unit_category_get_units(category); unit_iter;
       unit_iter = unit_iter->next) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0,
                       unit_get_display_name(u), 1, category, 2, u, -1);
  }
...
static void to_combobox_changed_cb(GtkWidget *combo, MathConverter *converter) {
...
static void from_cell_data_func(GtkCellLayout *cell_layout,
                                GtkCellRenderer *cell, GtkTreeModel *tree_model,
                                GtkTreeIter *iter, gpointer data) {
  g_object_set(cell, "sensitive",
               !gtk_tree_model_iter_has_child(tree_model, iter), NULL);
}

static void currency_updated_cb(CurrencyManager *manager,
                                MathConverter *converter) {
  update_result_label(converter);
}

static void swap_button_clicked_cb(GtkButton *button,
                                   MathConverter *converter) {
...
static void math_converter_init(MathConverter *converter) {
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(converter->priv->from_combo),
                             renderer, TRUE);
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(converter->priv->from_combo),
                                renderer, "text", 0);
  gtk_cell_layout_set_cell_data_func(
      GTK_CELL_LAYOUT(converter->priv->from_combo), renderer,
      from_cell_data_func, NULL, NULL);
  g_signal_connect(converter->priv->from_combo, "changed",
                   G_CALLBACK(from_combobox_changed_cb), converter);
  gtk_widget_show(converter->priv->from_combo);
  gtk_box_pack_start(GTK_BOX(hbox), converter->priv->from_combo, FALSE, TRUE,
                     0);

  label = gtk_label_new(/* Label that is displayed between the two conversion
                           combo boxes, e.g. "[degrees] in [radians]" */
                        _(" in "));
  ...
  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(converter->priv->to_combo),
                             renderer, TRUE);
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(converter->priv->to_combo),
                                renderer, "text", 0);
  g_signal_connect(converter->priv->to_combo, "changed",
                   G_CALLBACK(to_combobox_changed_cb), converter);
  ...
  g_signal_connect(swap_button, "clicked", G_CALLBACK(swap_button_clicked_cb),
                   converter);
  ...
  gtk_box_pack_start(GTK_BOX(converter), converter->priv->result_label, TRUE,
                     TRUE, 0);

  g_signal_connect(currency_manager_get_default(), "updated",
                   G_CALLBACK(currency_updated_cb), converter);
}
