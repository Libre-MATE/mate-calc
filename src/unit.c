...
gboolean unit_matches_symbol(Unit *unit, const gchar *symbol) {
  ...
  for (iter = unit->priv->symbols; iter; iter = iter->next) {
    gchar *s = iter->data;
    if (strcmp(s, symbol) == 0) return TRUE;
  }
...
static int variable_is_defined(const char *name, void *data) { return TRUE; }
...
static gboolean solve_function(const gchar *function, const MPNumber *x,
                               MPNumber *z) {
...
gboolean unit_convert_from(Unit *unit, const MPNumber *x, MPNumber *z) {
  ...
  if (unit->priv->from_function)
    ...
    const MPNumber *r;
    r = currency_manager_get_value(currency_manager_get_default(),
                                   unit->priv->name);
    if (!r) return FALSE;
    mp_divide(x, r, z);
...
static void unit_init(Unit *unit) {
  ...
  unit->priv->serializer =
      mp_serializer_new(MP_DISPLAY_FORMAT_AUTOMATIC, 10, 2);
