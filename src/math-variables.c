...
static void registers_load(MathVariables *variables) {
  FILE *f;
  char line[1024];
...
static void registers_save(MathVariables *variables) {
  gchar *dir;
  FILE *f;
  GHashTableIter iter;
  gpointer key, val;
...
gchar **math_variables_get_names(MathVariables *variables) {
  ...
  names = g_malloc0(sizeof(gchar *) *
                    (g_hash_table_size(variables->priv->registers) + 1));
...
static void math_variables_init(MathVariables *variables) {
  variables->priv = math_variables_get_instance_private(variables);
  variables->priv->registers =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
  variables->priv->file_name =
      g_build_filename(g_get_user_data_dir(), "mate-calc", "registers", NULL);
  variables->priv->serializer =
      mp_serializer_new(MP_DISPLAY_FORMAT_SCIENTIFIC, 10, 50);
