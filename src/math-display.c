...
MathDisplay *math_display_new() {
  return g_object_new(math_display_get_type(), "equation", math_equation_new(),
                      NULL);
}
...
static gboolean display_key_press_cb(GtkWidget *widget, GdkEventKey *event,
                                     MathDisplay *display) {
  int state;
  guint32 c;
  guint new_keyval = 0;
  ...
  switch (event->keyval) {
    ...
    case GDK_KEY_KP_Left:
      ...
      break;
    case GDK_KEY_KP_Begin: /* This is apparently what "5" does when numlock is
                              off. */
  ...
  if (state == GDK_CONTROL_MASK ||
      math_equation_get_number_mode(display->priv->equation) == SUPERSCRIPT) {
    switch (event->keyval) {
      case GDK_KEY_0:
      case GDK_KEY_KP_0:
        math_equation_insert(display->priv->equation, "⁰");
      ...
      case GDK_KEY_KP_9:
        ...
        return TRUE;
             ...
             math_equation_get_number_mode(display->priv->equation) ==
                 SUBSCRIPT) {
    switch (event->keyval) {
      case GDK_KEY_0:
      case GDK_KEY_KP_0:
...
static gboolean key_press_cb(MathDisplay *display, GdkEventKey *event) {
  gboolean result;
...
static void status_changed_cb(MathEquation *equation, GParamSpec *spec,
                              MathDisplay *display) {
  gtk_text_buffer_set_text(display->priv->info_buffer,
                           math_equation_get_status(equation), -1);
  if (math_equation_in_solve(equation) &&
      !gtk_widget_get_visible(display->priv->spinner)) {
    gtk_widget_show(display->priv->spinner);
    gtk_spinner_start(GTK_SPINNER(display->priv->spinner));
  } else if (!math_equation_in_solve(equation) &&
             gtk_widget_get_visible(display->priv->spinner)) {
...
static void _text_view_override_font(GtkWidget *widget,
...
static void create_gui(MathDisplay *display) {
  ...
  g_signal_connect(display, "key-press-event", G_CALLBACK(key_press_cb),
                   display);

  display->priv->text_view =
      gtk_text_view_new_with_buffer(GTK_TEXT_BUFFER(display->priv->equation));
  ...
  gtk_text_view_set_pixels_below_lines(GTK_TEXT_VIEW(display->priv->text_view),
                                       2);
  /* TEMP: Disabled for now as GTK+ doesn't properly render a right aligned
   * right margin, see bug #482688 */
  /*gtk_text_view_set_right_margin(GTK_TEXT_VIEW(display->priv->text_view),
   * 6);*/
  ...
  atk_object_set_role(gtk_widget_get_accessible(display->priv->text_view),
                      ATK_ROLE_EDITBAR);
  // FIXME:<property name="AtkObject::accessible-description" translatable="yes"
  // comments="Accessible description for the area in which results are
  // displayed">Result Region</property>
  g_signal_connect(display->priv->text_view, "key-press-event",
                   G_CALLBACK(display_key_press_cb), display);
  gtk_box_pack_start(GTK_BOX(main_box), display->priv->text_view, TRUE, TRUE,
                     0);
  ...
  gtk_widget_set_can_focus(
      info_view, TRUE);  // FIXME: This should be FALSE but it locks the cursor
                         // inside the main view for some reason
  gtk_text_view_set_cursor_visible(
      GTK_TEXT_VIEW(info_view), FALSE);  // FIXME: Just here so when incorrectly
                                         // gets focus doesn't look editable
  ...
  g_signal_connect(display->priv->equation, "notify::status",
                   G_CALLBACK(status_changed_cb), display);
  status_changed_cb(display->priv->equation, NULL, display);
}

static void math_display_set_property(GObject *object, guint prop_id,
...
static void math_display_get_property(GObject *object, guint prop_id,
...
static void math_display_class_init(MathDisplayClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("equation", "equation", "Equation being displayed",
                          math_equation_get_type(),
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void math_display_init(MathDisplay *display) {
