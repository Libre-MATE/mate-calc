...
void mperr(const char *format, ...) {
  ...
  if (mp_error) free(mp_error);
  mp_error = strdup(text);
}

const char *mp_get_error() { return mp_error; }
...
void mp_get_i(MPNumber *z) { mpc_set_si_si(z->num, 0, 1, MPC_RNDNN); }
...
bool mp_is_integer(const MPNumber *x) {
  if (mp_is_complex(x)) return false;
...
void mp_ln(const MPNumber *x, MPNumber *z) {
  ...
  /*if (mp_is_negative(x)) {
      // Translators: Error displayed attempted to take logarithm of negative
  value mperr(_("Logarithm of negative values is undefined"));
      mp_set_from_integer(0, z);
      return;
  }*/
...
void mp_reciprocal(const MPNumber *x, MPNumber *z) {
  mpc_t temp;
...
void mp_root(const MPNumber *x, long n, MPNumber *z) {
  ...
  if (n < 0) {
    ...
    mp_set_from_mp(x, z);
    p = n;
  } else { /* Translators: Error displayed when attempting to take zeroth root
            */
...
void mp_sqrt(const MPNumber *x, MPNumber *z) { mp_root(x, 2, z); }
...
void mp_factorial(const MPNumber *x, MPNumber *z) {
  ...
  if (!mp_is_natural(x)) {
    ...
    if (mp_is_negative(x) ||
        mp_is_complex(x)) { /* Translators: Error displayed when attempted take
                               the factorial of a negative or complex number */
    ...
    /* Factorial(x) = Gamma(x+1) - This is the formula used to calculate
     * Factorial of positive real numbers.*/
    mpfr_gamma(tmp2, mpc_realref(tmp.num), MPFR_RNDN);
    mpc_set_fr(z->num, tmp2, MPC_RNDNN);
    mp_clear(&tmp);
    mpfr_clear(tmp2);
  } else {
    /* Convert to integer - if couldn't be converted then the factorial would be
     * too big anyway */
...
void mp_modulus_divide(const MPNumber *x, const MPNumber *y, MPNumber *z) {
  if (!mp_is_integer(x) ||
      !mp_is_integer(y)) { /* Translators: Error displayed when attemping to do
                              a modulus division on non-integer numbers */
...
void mp_modular_exponentiation(const MPNumber *x, const MPNumber *y,
                               const MPNumber *p, MPNumber *z) {
...
void mp_xpowy(const MPNumber *x, const MPNumber *y, MPNumber *z) {
  /* 0^-n invalid */
  if (mp_is_zero(x) &&
      mp_is_negative(y)) { /* Translators: Error displayed when attempted to
                              raise 0 to a negative exponent */
...
void mp_xpowy_integer(const MPNumber *x, long n, MPNumber *z) {
  /* 0^-n invalid */
  if (mp_is_zero(x) && n < 0) { /* Translators: Error displayed when attempted
                                   to raise 0 to a negative re_exponent */
    mperr(_("The power of zero is undefined for a negative exponent"));
    mp_set_from_integer(0, z);
    return;
  }
...
void mp_erf(const MPNumber *x, MPNumber *z) {
  if (mp_is_complex(x)) { /* Translators: Error displayed when error function
                             (erf) value is undefined */
...
void mp_zeta(const MPNumber *x, MPNumber *z) {
  ...
  if (mp_is_complex(x) ||
      mp_compare(x, &one) == 0) { /* Translators: Error displayed when zeta
 ...
 */
static bool mp_is_pprime(MPNumber *n, ulong rounds) {
  MPNumber tmp = mp_new();
 ...
 */
static void mp_gcd(const MPNumber *a, const MPNumber *b, MPNumber *z) {
 ...
 */
static bool mp_pollard_rho(const MPNumber *n, ulong i, MPNumber *z) {
  MPNumber one = mp_new_from_unsigned_integer(1);
  MPNumber two = mp_new_from_unsigned_integer(2);
 ...
 */
static void find_big_prime_factor(const MPNumber *n, MPNumber *z) {
  MPNumber tmp = mp_new();
  ulong i = 2;
...
GList *mp_factorize(const MPNumber *x) {
  ...
  /* If value < 2^64-1, call for factorize_uint64 function which deals in
   * integers */
  uint64_t num = 1;
  num = num << 63;
  num += (num - 1);
  ...
  if (mp_is_less_equal(x, &int_max)) {
    ...
    if (mp_is_negative(x)) mp_invert_sign(list->data, list->data);
    mp_clear(&value);
    mp_clear(&tmp);
    mp_clear(&int_max);
    return list;
