...
static const char *error_code_to_string(MPErrorCode error) {
  ...
  if (error != PARSER_ERR_MP) return mp_error_code_to_string(error);
...
static void Test(char *expression, char *expected, int expected_error,
  ...
  if (error_token) g_free(error_token);

  if (error == 0) {
    char *result_str;
    MpSerializer *serializer;

    serializer = mp_serializer_new(MP_DISPLAY_FORMAT_FIXED, options.base,
                                   trailing_digits);
    result_str = mp_serializer_to_string(serializer, &result);
    g_object_unref(serializer);

    if (expected_error != PARSER_ERR_NONE)
      fail("'%s' -> %s, expected error %s", expression, result_str,
...
static int do_convert(const MPNumber *x, const char *x_units,
                      const char *z_units, MPNumber *z, void *data) {
...
static void try(const char *string, bool result, bool expected) {
  if ((result && !expected) || (!result && expected)) {
    fail("%s -> %s, expected %s", string, expected ? "true" : "false",
         result ? "true" : "false");
  } else {
...
static void test_mp() {
  ...
  try("mp_is_greater_than(-1, -1)", mp_is_greater_than(&minus_one, &minus_one),
      false);
  try("mp_is_greater_than(-1, 0)", mp_is_greater_than(&minus_one, &zero),
      false);
  try("mp_is_greater_than(-1, 1)", mp_is_greater_than(&minus_one, &one), false);

  try("mp_is_greater_equal(0, -1)", mp_is_greater_equal(&zero, &minus_one),
      true);
  ...
  try("mp_is_greater_equal(1, -1)", mp_is_greater_equal(&one, &minus_one),
      true);
  ...
  try("mp_is_greater_equal(-1, -1)",
      mp_is_greater_equal(&minus_one, &minus_one), true);
  try("mp_is_greater_equal(-1, 0)", mp_is_greater_equal(&minus_one, &zero),
      false);
  try("mp_is_greater_equal(-1, 1)", mp_is_greater_equal(&minus_one, &one),
      false);
  ...
  try("mp_is_less_than(-1, -1)", mp_is_less_than(&minus_one, &minus_one),
      false);
  ...
  try("mp_is_less_equal(-1, -1)", mp_is_less_equal(&minus_one, &minus_one),
      true);
...
static void test_equations(void) {
  ...
  test("1", "1", 0);
  test("1₁₆", "1", 0);
  test("2₂", "", PARSER_ERR_INVALID);
  test("2₈", "2", 0);
  test("2", "2", 0);
  test("2₁₆", "2", 0);
  test("3₂", "", PARSER_ERR_INVALID);
  test("3₈", "3", 0);
  test("3", "3", 0);
  test("3₁₆", "3", 0);
  test("4₂", "", PARSER_ERR_INVALID);
  test("4₈", "4", 0);
  test("4", "4", 0);
  test("4₁₆", "4", 0);
  test("5₂", "", PARSER_ERR_INVALID);
  test("5₈", "5", 0);
  test("5", "5", 0);
  test("5₁₆", "5", 0);
  test("6₂", "", PARSER_ERR_INVALID);
  test("6₈", "6", 0);
  test("6", "6", 0);
  test("6₁₆", "6", 0);
  test("7₂", "", PARSER_ERR_INVALID);
  test("7₈", "7", 0);
  test("7", "7", 0);
  test("7₁₆", "7", 0);
  ...
  test("d₁₆", "13", 0);
  test("e₂", "", PARSER_ERR_UNKNOWN_VARIABLE);
  test("e₈", "", PARSER_ERR_UNKNOWN_VARIABLE); /* e is a built-in variable */
  test("e₁₆", "14", 0);
  test("f₂", "", PARSER_ERR_UNKNOWN_VARIABLE);
  test("f₈", "", PARSER_ERR_UNKNOWN_VARIABLE);
  test("f", "", PARSER_ERR_UNKNOWN_VARIABLE);
  ...
  /*
      ...
      //test("2p", "0.000000000002", 0); // FIXME: Need to print out significant
     figures, not decimal places
      //test("2f", "0.000000000000002", 0); // FIXME: Need to print out
     significant figures, not decimal places
      ...
      //test("2n3", "0.0000000023", 0); // FIXME: Need to print out significant
     figures, not decimal places
      //test("2p3", "0.0000000000023", 0); // FIXME: Need to print out
     significant figures, not decimal places
      //test("2f3", "0.0000000000000023", 0); // FIXME: Need to print out
     significant figures, not decimal places
  */
  ...
  test("5!", "120", 0);
  test("69!",
       "17112245242814131137246833888127283909227054489352036939364804092325727"
       "9754140647424000000000000000",
       0);
...
int main(void) {
  ...
  test_conversions();
  test_equations();
  if (fails == 0) printf("Passed all %i tests\n", passes);
