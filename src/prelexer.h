...
typedef struct {
  ...
  guint
      length; /* Length of input string; stored to reduce calls to strlen(). */
  guint next_index; /* Index of next (to be read) character from input. */
  guint mark_index; /* Location, last marked. Useful for getting substrings as
                       part of highlighting */
} PreLexerState;
...
typedef enum {
  ...
  T_SUBTRACT,  // Minus
  T_MULTIPLY,  // Multiply
  T_DIV,       // Divide (note can't use T_DIVIDE as it is defined in *BSD as an
          // "integer divide fault" trap type value)
  T_MOD,          // Modulus
