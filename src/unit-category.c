...
void unit_category_add_unit(UnitCategory *category, Unit *unit) {
  ...
  category->priv->units =
      g_list_append(category->priv->units, g_object_ref(unit));
}

Unit *unit_category_get_unit_by_name(UnitCategory *category,
                                     const gchar *name) {
  GList *iter;
  ...
  for (iter = category->priv->units; iter; iter = iter->next) {
    Unit *unit = iter->data;
    if (strcmp(unit_get_name(unit), name) == 0) return unit;
  }
...
Unit *unit_category_get_unit_by_symbol(UnitCategory *category,
  ...
  for (iter = category->priv->units; iter; iter = iter->next) {
    Unit *unit = iter->data;
    if (unit_matches_symbol(unit, symbol)) return unit;
  }
...
static void unit_category_init(UnitCategory *category) {
