...
static void scroll_bottom_cb(MathHistory *history, gpointer data) {
  GtkAdjustment *adjustment;
  // TODO make this dynamic, do not hardcode
  // listbox_height_request/number_of_rows
  int width, height;

  adjustment =
      gtk_list_box_get_adjustment(GTK_LIST_BOX(history->priv->listbox));
...
void math_history_insert_entry(MathHistory *history, char *equation,
                               MPNumber *answer) {
  if (g_strcmp0(history->priv->last_equation, equation) == 0 &&
      history->priv->rows >= 1) {
    return;
  }
  ...
  math_history_entry_insert_entry(entry, equation, answer,
                                  history->priv->serializer);

  gtk_list_box_insert(GTK_LIST_BOX(history->priv->listbox), GTK_WIDGET(entry),
                      -1);
...
gpointer math_history_get_entry_at(MathHistory *history, int index) {
  if (index >= 0 && index < history->priv->rows)
    return MATH_HISTORY_ENTRY(gtk_list_box_get_row_at_index(
        GTK_LIST_BOX(history->priv->listbox), index));
  return NULL;
}

void math_history_set_current(MathHistory *history, int value) {
  history->priv->current =
      CLAMP(history->priv->current + value, 0, history->priv->rows - 1);
}
...
static void math_history_class_init(MathHistoryClass *klass) {
  g_signal_new("row-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, 0,
               NULL, NULL, NULL, G_TYPE_NONE, 0, NULL);
}

static void math_history_init(MathHistory *history) {
  history->priv = math_history_get_instance_private(history);

  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(history),
                                      GTK_SHADOW_IN);
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(history), GTK_POLICY_NEVER,
                                 GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_placement(GTK_SCROLLED_WINDOW(history),
                                    GTK_CORNER_TOP_LEFT);
  ...
  gtk_list_box_set_selection_mode(GTK_LIST_BOX(history->priv->listbox),
                                  GTK_SELECTION_NONE);
