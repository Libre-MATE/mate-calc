...
typedef enum { LEFT_ASSOCIATIVE, RIGHT_ASSOCIATIVE } Associativity;
...
typedef enum {
  ...
  P_Factorial = 11,
  P_NumberVariable = 12,
  /* P_Depth should be always at the bottom. It stops node jumping off the
     current depth level. */
  P_Depth
} Precedence;
...
typedef struct parser_state {
  ...
  int (*get_variable)(struct parser_state *state, const char *name,
                      MPNumber *z);
  void (*set_variable)(struct parser_state *state, const char *name,
                       const MPNumber *x);
  int (*function_is_defined)(struct parser_state *state, const char *name);
  int (*get_function)(struct parser_state *state, const char *name,
                      const MPNumber *x, MPNumber *z);
  int (*convert)(struct parser_state *state, const MPNumber *x,
                 const char *x_units, const char *z_units, MPNumber *z);
} ParserState;
