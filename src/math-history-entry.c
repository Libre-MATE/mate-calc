...
G_DEFINE_TYPE_WITH_PRIVATE(MathHistoryEntry, math_history_entry,
                           GTK_TYPE_LIST_BOX_ROW);

#define UI_HISTORY_ENTRY_RESOURCE_PATH \
  "/org/mate/calculator/ui/history-entry.ui"

MathHistoryEntry *math_history_entry_new(MathEquation *equation) {
  MathHistoryEntry *history_entry =
      g_object_new(math_history_entry_get_type(), NULL);
  history_entry->priv->equation = g_object_ref(equation);
  return history_entry;
}

void answer_clicked_cb(GtkWidget *widget, GdkEventButton *eventbutton,
                       MathHistoryEntry *history_entry);
G_MODULE_EXPORT
void answer_clicked_cb(GtkWidget *widget, GdkEventButton *eventbutton,
                       MathHistoryEntry *history_entry) {
  const gchar *answer =
      gtk_widget_get_tooltip_text(history_entry->priv->answer_label);

  if (answer != NULL)
    math_equation_insert(history_entry->priv->equation, answer);
}

void equation_clicked_cb(GtkWidget *widget, GdkEventButton *eventbutton,
                         MathHistoryEntry *history_entry);
G_MODULE_EXPORT
void equation_clicked_cb(GtkWidget *widget, GdkEventButton *eventbutton,
                         MathHistoryEntry *history_entry) {
  const gchar *equation =
      gtk_label_get_text(GTK_LABEL(history_entry->priv->equation_label));
...
void math_history_entry_redisplay(MathHistoryEntry *history_entry,
                                  MpSerializer *serializer) {
  gchar *answer =
      mp_serializer_to_string(serializer, history_entry->priv->number);
...
static void math_history_entry_finalize(GObject *object) {
...
static void math_history_entry_class_init(MathHistoryEntryClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->finalize = math_history_entry_finalize;
}

static void math_history_entry_init(MathHistoryEntry *history_entry) {
