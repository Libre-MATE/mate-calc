...
void mp_set_from_double(double dx, MPNumber *z) {
...
void mp_set_from_fraction(long numerator, long denominator, MPNumber *z) {
  ...
  if (denominator != 1) mp_divide_integer(z, denominator, z);
}

void mp_set_from_polar(const MPNumber *r, MPAngleUnit unit,
                       const MPNumber *theta, MPNumber *z) {
  MPNumber x = mp_new();
  MPNumber y = mp_new();
...
void mp_set_from_random(MPNumber *z) { mp_set_from_double(drand48(), z); }
...
static int char_val(char **c, int base) {
  int i, j, value;
  size_t offset;
  ...
  if (**c >= '0' && **c <= '9') {
    ...
    for (i = 0; digits[i][0]; i++) {
      for (j = 0; j < 10; j++) {
        if (strncmp(*c, digits[i][j], strlen(digits[i][j])) == 0) break;
      }
      if (j != 10) break;
    }
    if (digits[i][0] == NULL) return -1;
    value = j;
...
static int ends_with(const char *start, const char *end, const char *word) {
  size_t word_len = strlen(word);

  if (word_len > end - start) return 0;

  return strncmp(end - word_len, word, word_len) == 0;
}

// FIXME: Doesn't handle errors well (e.g. trailing space)
static bool set_from_sexagesimal(const char *str, int length, MPNumber *z) {
...
bool mp_set_from_string(const char *str, int default_base, MPNumber *z) {
  ...
  while (1) {
    ...
    if (base_digits[i] == NULL) break;
  }
  if (base_multiplier == 1) base = default_base;
  ...
  if (negate == 1) mp_invert_sign(z, z);
